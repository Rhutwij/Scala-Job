...
class TransformSpec extends FunSpec with BeforeAndAfter with SharedSparkContext with RedisConfig with HashFunctions{
  ...
  describe("UserProfileApp") {
    ...
    it("should read pubMailUpdateDataFrame and return results total below") {
      val profiles: DataFrame = FunctionLib.getPubMailDataFrame(pubMailUpdateDataFrame)
    ...
    it("should read mailUpdateDataFrame and return results total below") {
      val profiles: DataFrame = FunctionLib.getMailDataFrame(mailUpdateDataFrame)
    ...
    it("should read transform getPubMailDataFrame impressions in RDD[(String, Seq[MailImpressions])] ") {
      val profiles: RDD[(String, Seq[MailImpressions])] = FunctionLib.transformImpressions(FunctionLib.getPubMailDataFrame(pubMailUpdateDataFrame))
    ...
    it("should read transform MailDataFrame impressions in RDD[(String, Seq[MailImpressions])] ") {
      val profiles: RDD[(String, Seq[MailImpressions])] = FunctionLib.transformImpressions(FunctionLib.getMailDataFrame(mailUpdateDataFrame))
    ...
    it("unionImpressionsRDD test the total should be equal ") {
      val profiles1: RDD[(String, Seq[MailImpressions])] = FunctionLib.transformImpressions(FunctionLib.getMailDataFrame(mailUpdateDataFrame))
      val profiles2: RDD[(String, Seq[MailImpressions])] = FunctionLib.transformImpressions(FunctionLib.getPubMailDataFrame(pubMailUpdateDataFrame))
      val unionedProfiles = FunctionLib.unionImpressionsRDD(profiles1, profiles2)
      unionedProfiles.count() should be(53)
    }
    //Test5.1
    it("unionImpressionsRDD NULL test the total should be equal ") {
      val profiles1: RDD[(String, Seq[MailImpressions])] = FunctionLib.transformImpressions(FunctionLib.getPubMailDataFrame(pubMailUpdateDataFrame))
    ...
    it("Impressions Reduction with ONE NULL RDD") {
      val profiles1: RDD[(String, Seq[MailImpressions])] = FunctionLib.transformImpressions(FunctionLib.getPubMailDataFrame(pubMailUpdateDataFrame))
      val profiles2: RDD[(String, Seq[MailImpressions])] = sc.emptyRDD
      val unionedProfiles: RDD[(String, Seq[MailImpressions])] = FunctionLib.unionImpressionsRDD(profiles1, profiles2)
      val reducedRDD: RDD[(String, Seq[MailImpressions])]=FunctionLib.impressionReduction(unionedProfiles)
    ...
    it("Impressions Reduction with NO NULL") {
      val profiles1: RDD[(String, Seq[MailImpressions])] = FunctionLib.transformImpressions(FunctionLib.getPubMailDataFrame(pubMailUpdateDataFrame))
      val profiles2: RDD[(String, Seq[MailImpressions])] = FunctionLib.transformImpressions(FunctionLib.getMailDataFrame(mailUpdateDataFrame))
      val unionedProfiles: RDD[(String, Seq[MailImpressions])] = FunctionLib.unionImpressionsRDD(profiles1, profiles2)
      val reducedRDD: RDD[(String, Seq[MailImpressions])]=FunctionLib.impressionReduction(unionedProfiles)
    ...
    it("Grouping impressions by time test") {
      ...
      val unionedProfiles: RDD[(String, Seq[MailImpressions])] = FunctionLib.unionImpressionsRDD(profiles1, profiles2)
      val reducedRDD: RDD[(String, Seq[MailImpressions])]=FunctionLib.impressionReduction(unionedProfiles)
      val groupedRDD: RDD[UserProfile]=FunctionLib.groupImpressionsBySentTime(reducedRDD)
